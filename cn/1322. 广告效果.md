...
```sql
SELECT ad_id, ROUND(IFNULL(COUNT(IF(action = 'Clicked', TRUE, NULL)) * 100 / (COUNT(IF(action = 'Viewed', TRUE, NULL)) + COUNT(IF(action = 'Clicked', TRUE, NULL))), 0), 2) ctr 
FROM
	Ads a GROUP BY a.ad_id ORDER BY ctr DESC, ad_id;
```
