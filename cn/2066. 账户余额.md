...
```sql
WITH t AS (
SELECT account_id, day, type, if(type = 'Deposit', amount, -amount) value FROM transactions
)
SELECT 
	account_id, 
	day, 
	SUM(value) OVER (PARTITION BY account_id ORDER BY day ROWS UNBOUNDED PRECEDING) AS balance 
FROM t;
```
